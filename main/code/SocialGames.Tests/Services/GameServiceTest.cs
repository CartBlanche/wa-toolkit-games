namespace Microsoft.Samples.SocialGames.Tests
{
    ...
    public class GameServiceTest : ServiceTest
    {
        ...
        public void SetWeapons()
        {
            ...
            var userRepository = this.CreateUserRepository();
            var gameRepository = this.CreateGameRepository();
            Game game = this.CreateNewGame(gameRepository, userId, "peter", "anna");

            GameService gameService = this.CreateGameService(gameRepository, userRepository, userId);

            var request = new HttpRequestMessage();

            Guid weaponId1 = Guid.NewGuid();
            Guid weaponId2 = Guid.NewGuid();
            Guid weaponId3 = Guid.NewGuid();

            request.Content = new StringContent(string.Format(CultureInfo.InvariantCulture, "weaponIds[]={0}&weaponIds[]={1}&weaponIds[]={2}", weaponId1, weaponId2, weaponId3));

            HttpResponseMessage response = gameService.SetWeapons(game.Id, request);

            Game updatedGame = gameRepository.GetGame(game.Id);

            Assert.IsNotNull(updatedGame);

            GameUser gameUser = updatedGame.Users.Where(ug => ug.UserId == userId).FirstOrDefault();

            Assert.IsNotNull(gameUser);
            Assert.IsNotNull(gameUser.Weapons);
            Assert.AreEqual(3, gameUser.Weapons.Count);

            Assert.IsTrue(gameUser.Weapons.Contains(weaponId1));
            Assert.IsTrue(gameUser.Weapons.Contains(weaponId2));
            Assert.IsTrue(gameUser.Weapons.Contains(weaponId3));
        }

        [TestMethod]
        public void BadRequestIfUserIsNotInGameWhenSetWeapons()
        {
            var userId = "johnny";
            var userRepository = this.CreateUserRepository();
            var gameRepository = this.CreateGameRepository();
            Game game = this.CreateNewGame(gameRepository, "john", "peter", "anna");
        ...
        [TestMethod]
        public void BadRequestIfGameDoesNotExistWhenSetWeapons()
        {
            var userId = "johnny";
            var userRepository = this.CreateUserRepository();
            var gameRepository = this.CreateGameRepository();
            ...
            HttpResponseMessage response = gameService.SetWeapons(Guid.NewGuid(), request);

            Assert.AreEqual(HttpStatusCode.BadRequest, response.StatusCode);
            Assert.IsTrue(response.Content.ReadAsString().StartsWith("Game does not exist"));
        }
